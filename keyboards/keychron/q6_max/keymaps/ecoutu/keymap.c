...
enum custom_keycodes { KC_W_HOLD = SAFE_RANGE, KC_Q_HOLD };
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [WIN_FN] = LAYOUT_109_ansi(
        ...
        _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,    _______,    _______,  _______,  _______,  _______,  _______,  _______,  _______,
        _______,  KC_Q_HOLD,  KC_W_HOLD,    _______,    _______,    _______,    _______,  _______,  _______,  _______,  _______,  _______,  _______,    _______,    _______,  _______,  _______,  _______,  _______,  _______,
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  switch (keycode) {
  case KC_W_HOLD:
    if (record->event.pressed && !w_held) {
      register_code(KC_W);
      w_held = true;
    }
    return false;
  case KC_W:
    ...
    return true;
  case KC_Q_HOLD:
    if (record->event.pressed && !q_held) {
      register_code(KC_Q);
      q_held = true;
    }
    return false;
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
  ...
  for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
    for (uint8_t col = 0; col < MATRIX_COLS; col++) {
      ...
      if (led_i == NO_LED || led_i < led_min || led_i >= led_max)
        continue;
      keypos_t pos = {.row = row, .col = col};
      uint16_t keycode = keymap_key_to_keycode(WIN_BASE, pos);
      if ((w_held && (keycode == KC_W_HOLD || keycode == KC_W)) || (q_held && (keycode == KC_Q_HOLD || keycode == KC_Q))) {
